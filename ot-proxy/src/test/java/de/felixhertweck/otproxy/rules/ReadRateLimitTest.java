...
class ReadRateLimitTest {
    ...
    void allowsReadsWhenNoReadLimitConfigured() {
        ...
        for (int i = 0; i < 100; i++) {
            assertThat(engine.evaluateRead(ADDR, Instant.now()).allowed()).isTrue();
        }
    }
    ...
    void allowsReadsUpToLimitThenBlocks() {
        ...
        for (int i = 0; i < 3; i++) {
            assertThat(engine.evaluateRead(ADDR, base.plusMillis(i)).allowed())
                    .as("read #" + i)
                    .isTrue();
        }
        RuleResult fourth = engine.evaluateRead(ADDR, base.plusMillis(3));
        assertThat(fourth.allowed()).isFalse();
        assertThat(fourth.reason()).contains("read register " + ADDR);
    ...
    void allowsReadAfterWindowExpires() {
        ...
        assertThat(engine.evaluateRead(ADDR, base).allowed()).isTrue();
        // 50ms later -> still inside the 100ms window -> blocked
        assertThat(engine.evaluateRead(ADDR, base.plusMillis(50)).allowed()).isFalse();
        // 150ms later -> window expired -> allowed again
        assertThat(engine.evaluateRead(ADDR, base.plusMillis(150)).allowed()).isTrue();
    }
    ...
    void readLimitsArePerRegister() {
        ...
        assertThat(engine.evaluateRead(ADDR, now).allowed()).isTrue();
        assertThat(engine.evaluateRead(ADDR, now).allowed()).isFalse();
        // a different register has its own independent window
        assertThat(engine.evaluateRead(ADDR + 1, now).allowed()).isTrue();
    }
    ...
    void registerReadLimitOverridesDefaultAndInheritsRegisterAction() {
        ...
        assertThat(engine.evaluateRead(ADDR, base).allowed()).isTrue();
        assertThat(engine.evaluateRead(ADDR, base.plusMillis(1)).allowed()).isTrue();
        RuleResult blocked = engine.evaluateRead(ADDR, base.plusMillis(2));
